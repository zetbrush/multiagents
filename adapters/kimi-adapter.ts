#!/usr/bin/env bun
/**
 * KimiAdapter — MCP adapter for Kimi CLI instances.
 *
...
export class KimiAdapter extends BaseAdapter {
  ...
  getSystemPrompt(): string {
    ...
    const base = `You are a Kimi K2.6 instance (Moonshot AI, reasoning always-on, 262K context window) on the multiagents network.
You are a team member on the multiagents network. You MUST use the multiagents-peer MCP tools to communicate. Your team CANNOT see your work unless you tell them.
...
QUALITY: Production-grade code. Plan before coding. Verify before signaling done.`;
   ...
   * the file-based inbox so Kimi sees it via native file reads.
   */
